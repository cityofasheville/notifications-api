-- Data Library internal.notification_permits source
-- This is copied nightly to the note.notification_permits table

CREATE OR REPLACE VIEW internal.notification_permits
AS SELECT notification_emails.permit_num,
    notification_emails.applied_date,
    notification_emails.name,
    notification_emails.x,
    notification_emails.y,
    jsonb_agg(notification_emails.tag) AS tags
   FROM ( SELECT 'Minor'::text AS tag,
        ...
        UNION
         SELECT 'SoundExceedance'::text AS tag,
           ...
           FROM internal.permits a
             LEFT JOIN internal.coa_bc_address_master b ON a.civic_address_id::text = b.civicaddress_id::character varying(50)::text
          WHERE a.permit_group::text = 'Permits'::text AND a.permit_type::text = 'Commercial'::text AND a.permit_subtype::text = 'Existing Building'::text AND a.permit_category::text = 'Sound Exceedance'::text) notification_emails
  GROUP BY notification_emails.name, notification_emails.permit_num, notification_emails.applied_date, notification_emails.x, notification_emails.y
 HAVING notification_emails.permit_num::text !~~ '%TMP%'::text;